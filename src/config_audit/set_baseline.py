"""Author a device's baseline directly from a config file, with no live device
pull first -- the missing piece for zero-touch provisioning (D6-adjacent).

promote.py's lifecycle (backup -> diff -> promote) assumes a live device already
exists to pull FROM. ZTP is the opposite: the device is blank/factory-default and
the desired config is authored ahead of time (hand-written, or a shared template
reused across several similar switches) -- there is no live state to promote from
yet. This module is that authoring step: it reads an arbitrary file (not
backup_dir) and produces a plan the CLI can show a human before writing.

plan_set_baseline() is pure -- no writes. The actual write reuses
promote.apply_promotion() (same operation: write raw text to
baseline_dir/<device>.cfg) rather than duplicating it -- the two commands differ
only in where the text comes from (an on-disk backup vs. an arbitrary file), not
in how it gets written.
"""
