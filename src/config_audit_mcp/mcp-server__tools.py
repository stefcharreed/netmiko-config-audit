...
from config_audit import drift, promote
...
def promote_baseline(device: str, confirm: bool = False,
    ...
    The git commit is left to the caller in the CLI, but here it is performed via
    config_audit so the MCP action is self-contained; callers should run this
    only against a baseline dir that is its own git repo.
    """
    cfg = _load(config_path)
    plan = promote.plan_promotion(device, cfg.settings.backup_dir, cfg.settings.baseline_dir)
    ...
    return {"device": device, "promoted": True, "baseline_path": str(path)}
