...
└── tests/                       # 82 tests (pytest); no live gear, no network
    ├── test_*.py                # Project 1 — 57 tests
...
82 tests cover the offline pipeline end to end (57 for the tool, 25 for the MCP adapter). They need no live gear, no network, and no Netmiko — the collector's `source_text` seam lets the whole pipeline run against saved configs, so the suite is pure and fast:
...
docker build --target test -t netmiko-audit:test .     # runs the 82-test suite inside the image; build fails on any failure
```
...
- [x] 82-test suite: 57 tool tests (phantom-drift guard, drift detection, promote gate, sanitizer, secrets wizard + confirmation) + 25 MCP adapter tests
